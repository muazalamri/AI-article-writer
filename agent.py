...
class Asseten:
    ...
    async def answer(self, prompt):
        ...
        answer = await self.genfun(f'''write articel about {prompt} :
Make it seo optimized
Write it as html div with bootstarp 5 classes
make it sturcerd and useful
write only article and no thing more
don't use (`)''')
...
async def AItopicer(prompt):
    print('prompt--------->',prompt)
    text=await asset.answer(prompt) 
    return {'res':text}
...
def my_blog():
    ...
    data = asyncio.run(AItopicer(request.form.get('topic')))['res'][7:-4].replace('\n','<br>')
